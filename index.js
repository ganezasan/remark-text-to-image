 ...
 * extractText.
 *
 * @param {string} string - string going to be extracted
 * @param {number} start - extract start position
 * @param {number} end - extract end position
 * @return {text} - Text.
 */
...
function transformer(tree) {
    visit(tree, 'text', function (node, position, parent) {
        var definition = [];
        var lastIndex = 0;
        var m;
        while ((m = regex.exec(node.value)) !== null) {
            ...
            if (m.index != lastIndex) {
                var text = extractText(node.value, lastIndex, m.index);
            ...
            lastIndex = m.index + m.length - 1;
        }
        if (lastIndex != node.value.length) {
            text = extractText(node.value, lastIndex, node.value.length);
            definition.push(text);
        }
        var last = parent.children.slice(position + 1);
